...
public class Parser {
	...
	public static Scene createScene(Node dataNode) {
        ...
        Node refRecNode = dataNode.getAttributes().getNamedItem("refRecursion");
		if ( refRecNode != null ) {
			Scanner s = new Scanner( refRecNode.getNodeValue() );
			scene.refRecursion = s.nextInt();
			s.close();
		}
	...
	public static Material createMaterial(Node dataNode) {
		...
		if( refAttr  != null ) {
	    	...
	    	Node refCoeffNode = dataNode.getAttributes().getNamedItem("refCoeff");
			if (refCoeffNode != null) {
				Scanner s = new Scanner( refCoeffNode.getNodeValue());
				float r = s.nextFloat();
				float g = s.nextFloat();
				float b = s.nextFloat();
				float a = (s.hasNextFloat() ? s.nextFloat() : 1);
